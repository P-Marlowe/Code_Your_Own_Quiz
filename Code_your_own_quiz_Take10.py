...
difficulty_level = ['easy', 'medium', 'hard'] #THIS IS NEW FOR V10
...
#def determine_level(user_input):
    ...
    #Input: 'user_input' string, which is user's preferred difficulty level
    #Output: integer [0-2] or None if no match
...
#strip_punctuation and determine_level ARE NEW FOR V10
...
def determine_level(user_input):
    ...
    user_input = strip_punctuation(user_input, difficulty_level)
    if user_input in difficulty_level:
        index_level = difficulty_level.index(user_input)
        return index_level
    else:
        return None
