...
#strip_punctuation and determine_level ARE NEW/REWRITTEN FOR V10
def strip_punctuation(user_input, difficulty_level):
    for element in difficulty_level:
        if element in user_input:
            return element

def determine_level(user_input):
    user_input = user_input.lower()
    user_input = strip_punctuation(user_input, quiz_level)
    if user_input in quiz_level:
        index_level = quiz_level.index(user_input)
        return index_level
    else:
        return None
...
def review_answer(answer_input, current_possible_answers, blanks):
    ...
    #Input: 'answer_input' string, which is user's suggested answer
    #Output: True if match; False if no match
    if answer_input in current_possible_answers[blanks]: #[determine_level(user_input)]
        return True
    return False


def replace_blanks(current_quiz, blanks_list, blanks, current_answers):
    #Behavior: replace blank(s) with answer
    #Inputs: (1)'current_quiz' string, which is string from 'quiz' list; (2) 'blanks_list' list
    #Output: returns 'replaced' string, which is modification of 'current_quiz' string
    replaced = []
...
def start_game():
    ...
    current_quiz = quiz[determine_level(user_input)]
    current_answers = answers[determine_level(user_input)]
    current_possible_answers = possible_answers[determine_level(user_input)]
    print "\n" + "You chose " + quiz_level[determine_level(user_input)] + "."
    print rules
